...
from evaluate import evaluate, _evaluate_casia_b, _evaluate_iLIDS, _evaluate_mars
...
def main(opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if opt.dataset == "iLIDS":
            result, accuracy_avg = evaluate(val_loader, model, opt.evaluation_fn, use_flip=True
    )
        if opt.dataset == "mars":
            result, accuracy_avg = evaluate(val_loader, model, opt.evaluation_fn, use_flip=True
    )
        ...
        if is_best:
            ...
            save_file = os.path.join(opt.save_folder, f"ckpt_epoch_best.pth")
...
if __name__ == "__main__":
    ...
    if opt.dataset== "iLIDS":
        opt.evaluation_fn= _evaluate_iLIDS
    if opt.dataset== "mars":
        opt.evaluation_fn= _evaluate_mars
    if opt.tune:
        tune_()
    else:
        main(opt)
